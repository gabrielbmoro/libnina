...
OPARI2_CTC_error(CTCData * obj,
		 CTC_ERROR_Type errorType, const char *info1)
{
  bool abort = true;
  ...
  case CTC_ERROR_Inconsistent_line_numbers:
    printf
	("Warning: line numbers not valid. Expected startLineNo1 <= startLineNo2 <= endLineNo1 <= endLineNo2 \n");
    abort = false;
    break;
...
static void initRegionInfo(OPARI2_Region_info * regionInfo);

static void
copyCTCStringToInternalMemory(CTCData * obj, const char *source);

void OPARI2_CTC_initCTCData(CTCData * obj, const char string[])
{
...
void OPARI2_CTC_freeCTCData(CTCData * obj)
{
...
void OPARI2_CTC_freeAndReset(char **freeMe)
{
  if (*freeMe) {
    free(*freeMe);
    *freeMe = 0;
...
assignSourceCodeLocation(CTCData * obj,
			 char **fileName,
			 unsigned *line1, unsigned *line2, char *value);

static void ignoreLengthField(CTCData * obj);
...
static bool getKeyValuePair(CTCData * obj, char **key, char **value);
...
static bool extractNextToken(char **string, const char tokenDelimiter);
			 ...
			 unsigned *line1, unsigned *line2, char *value)
{
  ...
  if (*filename != 0 && line1Tmp > -1 && line2Tmp > -1) {
    ...
    *line2 = line2Tmp;
    if (*line1 > *line2) {
      OPARI2_CTC_error(obj, CTC_ERROR_SCL_line_number_error,
		       "line1 > line2");
    }
  } else {
...
OPARI2_CTC_assignHasClause(CTCData * obj,
			   bool * hasClause, const char *value)
{
  if (!isdigit(*value)) {
...
OPARI2_CTC_assignUnsigned(CTCData * obj,
			  unsigned *anUnsigned, const char *value)
{
  int tmp = atoi(value);
  if (tmp < 0) {
...
void OPARI2_CTC_assignString(char **aString, const char *value)
{
  *aString = malloc(strlen(value) * sizeof(char) + 1);
  strcpy(*aString, value);
}

void OPARI2_CTC_checkConsistency(CTCData * obj)
{
...
OPARI2_CTC_string2Enum(const OPARI2_CTCMapType * map,
		       const size_t n_elements, const char *string)
{
  unsigned int i;
...
const char *OPARI2_CTC_enum2String(const OPARI2_CTCMapType * map,
				   const size_t n_elements, int e_in)
{
  unsigned int i;
  for (i = 0; i < n_elements; ++i) {
    if (e_in == map[i].mEnum) {
