...
void model_read_configuration(void)
{
  ...
  model_error_check(config_lookup_string
		    (&cfg, "filename", &trace_filename_prefix));
...
#pragma omp parallel
  if (PAPI_thread_init((unsigned long (*)(void)) (omp_get_thread_num)) !=
      PAPI_OK) {
...
void model_papi_configuration(config_t * cfg)
{
  ...
  int i;
  for (i = 0;
       (papi_event_name = config_setting_get_string_elem(sub_cfg, i));
       i++) {
