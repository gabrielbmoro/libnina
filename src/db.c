...
void LIBNINA_LoadRegionsFile(void)
{
  ...
  for (i = 0; i < amountOfLines; i++) {
    ...
    for (j = 0; j < nina_regions_length; j++) {
      LOG(printf
	  ("Search for %s in %s\n", fileNameTmp,
	   nina_regions[j].source_filename));
    ...
    LOG(printf
	("Add %d line %ld freq to %s region\n", start_lineTmp, freqTmp,
	...
	LOG(printf
	    ("After ENTER, FIND returned [%s]=%ld\n", ep->key,
	     *(long *) ep->data));
	if (*(long *) ep->data == freqTmp) {
	  LOG(printf
	      ("Entered value equals the one that has been found, which is great.\n"));
	} else {
...
long LIBNINA_GetFrequency(char *source_filename, int start_line)
{
  ...
  for (j = 0; j < nina_regions_length; j++) {
    LOG(printf
	("%s Search for [%s] in %s\n", __FUNCTION__, source_filename,
	 nina_regions[j].source_filename));
    if (strstr(source_filename, nina_regions[j].source_filename)) {
      region = &nina_regions[j];
      LOG(printf
	  ("%s Found [%s] in %d (%s)\n", __FUNCTION__, source_filename, j,
	   nina_regions[j].source_filename));
      break;
    }
  }
  ...
  if (region == NULL) {
    fprintf(stderr, "Region with name [%d,FREQ?,%s] not found.\n",
	    start_line, source_filename);
    return -1;
  } else {
  ...
  if (ep == NULL) {
    LOG(printf
	("%s The line number %d (%s) has not been found in source file %s.\n",
