...
- Okay, it's possible to run some program using libnina. In previous
  ...
  libnina. Now, we're gonna replicate the same experiment, but we
  using the libnina with userspace governor on =hype2= platform.
  
**** Step1

- Is the same methodology used in =teste_1=.
...
- I used a MakeFile to compile strassen application with libnina:
...
- If you want to run strassen without libnina, you use =strassen_c=. If
  you want to run strassen with libnina, you use =strassen_c_inst=. 
